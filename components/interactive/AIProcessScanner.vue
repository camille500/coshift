  ...
  <div class="scanner-inner">
    <div class="scanner-chips">
      <button
        ...
        :key="cat"
        :class="['scanner-chip', { 'scanner-chip-active': selected === cat }]"
        @click="selected = cat"
      >
        <span v-html="CATEGORY_ICONS[cat - 1]" />
        {{ t(`scanner.cat.${cat}`) }}
      </button>
    </div>

    <div v-if="selected !== null" class="scanner-result glass">
      <div class="scanner-approach">
        <span class="scanner-approach-label">{{ t('scanner.result.approach') }}</span>
        <p class="scanner-approach-text">{{ t(`scanner.res.${selected}.approach`) }}</p>
      </div>
      <a href="#contact" class="scanner-cta">
        {{ t('scanner.cta') }}
...
.scanner-chip:hover { border-color: rgba(0, 212, 170, 0.2); color: var(--text-secondary); }
.scanner-chip-active { color: var(--accent); background: rgba(0, 212, 170, 0.08); border-color: rgba(0, 212, 170, 0.3); }
.scanner-chip-active:hover { color: var(--accent); border-color: rgba(0, 212, 170, 0.3); }
...
.scanner-approach { margin-bottom: 24px; }
.scanner-approach-label {
  font-size: 0.75rem; color: var(--text-muted); font-weight: 500;
  text-transform: uppercase; letter-spacing: 0.06em; display: block; margin-bottom: 8px;
}
.scanner-approach-text { color: var(--text-secondary); font-size: 0.95rem; line-height: 1.65; }
.scanner-cta {
  display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px;
  font-size: 0.88rem; font-weight: 600; color: var(--bg-deep); background: var(--accent);
  border-radius: 8px; transition: transform 0.25s ease, box-shadow 0.35s ease;
}
.scanner-cta:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0, 212, 170, 0.35); }
.scanner-cta svg { transition: transform 0.2s ease; }
.scanner-cta:hover svg { transform: translateX(3px); }
@media (max-width: 600px) {
  .scanner-chips { flex-direction: column; align-items: stretch; }
  .scanner-result { padding: 24px 20px; }
}
</style>
