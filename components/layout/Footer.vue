  ...
  <footer class="footer">
    <div class="footer-inner">
      ...
      <div class="footer-col">
        ...
        <NuxtLink :to="localePath('/#impact')">{{ t('nav.impact') }}</NuxtLink>
        <NuxtLink :to="localePath('/#use-cases')">{{ t('nav.usecases') }}</NuxtLink>
        <NuxtLink :to="localePath('/blog')">{{ t('nav.blog') }}</NuxtLink>
        <NuxtLink :to="localePath('/#contact')">{{ t('nav.contact') }}</NuxtLink>
      </div>

      <div class="footer-col">
        <h4 class="footer-heading">{{ t('footer.services') }}</h4>
        <NuxtLink :to="localePath('/#services')">{{ t('footer.s1') }}</NuxtLink>
        <NuxtLink :to="localePath('/#services')">{{ t('footer.s2') }}</NuxtLink>
        <NuxtLink :to="localePath('/#services')">{{ t('footer.s3') }}</NuxtLink>
        <NuxtLink :to="localePath('/#services')">{{ t('footer.s4') }}</NuxtLink>
        <NuxtLink :to="localePath('/#services')">{{ t('footer.s6') }}</NuxtLink>
      </div>

      <div class="footer-col footer-trust">
        <div class="trust-badge">{{ t('footer.data') }}</div>
        <div class="trust-badges">
          <span class="badge">{{ t('footer.gdpr') }}</span>
          <span class="badge">{{ t('footer.euai') }}</span>
    ...
    <div class="footer-bottom">
      <p>&copy; {{ year }} CoShift. {{ t('footer.rights') }}</p>
    </div>
  </footer>
</template>
...
<style scoped>
.footer { border-top: 1px solid var(--border-glass); padding: 64px 24px 32px; }
...
.footer-logo { font-family: var(--font-display); font-size: 1.35rem; font-weight: 800; letter-spacing: -0.04em; margin-bottom: 12px; }
...
.trust-badge { font-size: 0.875rem; color: var(--accent); font-weight: 600; margin-bottom: 8px; }
